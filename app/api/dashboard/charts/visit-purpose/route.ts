...
export async function GET(request: Request) {
  ...
  const items = await getDimensionMetrics({ facilityId, periodType, dimensionType: "visit_purpose", year, month });

  return NextResponse.json({
    dimensionType: "visit_purpose",
    items: items.map((i) => ({ label: toJapaneseLabel("visit_purpose", i.dimensionValue), usageCount: i.usageCount, percentage: i.percentage ? Number(i.percentage) : null })),
  });
}
