    ...
    <div className="flex min-h-screen">
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={cn(
                ...
                pathname === item.href ? "bg-secondary text-primary font-medium" : "text-muted-foreground hover:bg-muted"
              )}
            >
      ...
      <div className="flex flex-1 flex-col">
        <header className="flex h-16 items-center justify-between border-b border-border bg-white px-6">
          <div>
            <p className="text-sm font-medium">{session?.user?.facilityName || "Beauty Spot"}</p>
          </div>
